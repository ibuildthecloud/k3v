...
func runWithLogging(cfg *cmds.Server) error {
	l := &lumberjack.Logger{
		Filename:   cfg.Log,
		MaxSize:    50,
		MaxBackups: 3,
...
func Run(app *cli.Context) error {
	...
	return Start(ctx, app.GlobalBool("debug"), app.App.Version, &cmds.ServerConfig, cmds.AgentConfig)
}

func Start(ctx context.Context, debug bool, version string, cfg *cmds.Server, agentConfig cmds.Agent) error {
	var (
		err error
	)

	if cfg.Log != "" && os.Getenv("_RIO_REEXEC_") == "" {
		return runWithLogging(cfg)
	}
	...
	if agentConfig.FlannelIface != "" && agentConfig.NodeIP == "" {
		agentConfig.NodeIP = netutil.GetIPFromInterface(agentConfig.FlannelIface)
	}

	if serverConfig.ControlConfig.AdvertiseIP == "" && agentConfig.NodeIP != "" {
		serverConfig.ControlConfig.AdvertiseIP = agentConfig.NodeIP
	}
	...
	for _, noDeploy := range cfg.NoDeploy {
	...
	logrus.Info("Starting k3s ", version)
